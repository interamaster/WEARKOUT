...
public class CountDownAnimation {
	...
	private final Runnable mCountDown = new Runnable() {
		public void run() {
			if (mCurrentCount > 0) {
				...
				mCurrentCount--;
				//no sinventamos a un listener para que vibre cada seg de la cuneta atras!!!
				if (mListener != null)
					mListener.onCountDown1Segundo(CountDownAnimation.this);
			} else {
	 ...
	 */
	public CountDownAnimation(TextView textView, int startCount, int tag) {
	...
	public static interface CountDownListener {
		...
		void onCountDown1Segundo(CountDownAnimation countDownAnimation);
	}
}
